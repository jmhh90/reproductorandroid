...
public class Buscag extends ActionBarActivity {
	...
	public ListView lv;
	   public void onCreate(Bundle savedInstanceState) { 
	        ...
	        setContentView(R.layout.buscaguarda);
...
adView.loadAd(adRequest);
     ...
     nlista.setText(getString(R.string.no_lista));
      	//Se crea la base de datos si no estuviera hecha
           ...
           //Se llama al mantener pulsado un elemento de las listas guardadas
    		lv.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
    			...
    			protected boolean onLongListItemClick(View v, int pos, long id) {
    					...
    					//Llama al dialog de borrar las listas
	   ...
	   //Dialog que muestra añadir o borrar un elemento de la lista
    ...
    private void crearDialogo() {
	   ...
	   //Dialog que muestra borrar todas las listas

    private void crearDialogoBorrartodo() {
        new MaterialDialog.Builder(this)
                // .iconRes(R.drawable.dia_log)
                // .title(getString(R.string.tweetoface))
                .content(getString(R.string.borrar_todol))
                ...
                .callback(new MaterialDialog.ButtonCallback() {
                    @Override
                    public void onPositive(MaterialDialog dialog) {
                        //Llama al thread que borra todas las listas
	   ...
	   private class threadlista extends AsyncTask<Void, Void, Void> {
		   ProgressDialog pDialog = new ProgressDialog(Buscag.this);
		   ...
		   protected void onPreExecute() {
			...
			protected Void doInBackground(Void... params) {
				//Realiza la consulta a la base de datos con el nombre de la lista
			    		...
			    		Cursor p = db.rawQuery("SELECT ruta, cancion, grupo, tiempo FROM lista  WHERE nombre='"+name+"'",null);
	   ...
	   private class threadborrar extends AsyncTask<Void, Void, Void> {
		   ...
		   protected void onPreExecute() {
			...
			protected Void doInBackground(Void... params) {
	        	...
	        	String name=mostrar.get(posicion);
	        	db.execSQL("DELETE FROM lista WHERE nombre='"+name+"'");
	   ...
	   //Borra todas las listas guardadas
	   ...
	   private class threadbd extends AsyncTask<Void, Void, Void> {
		   ...
		   int mostr=0;
			...
			protected Void doInBackground(Void... params) {
	      	...
	      	int num=c.getInt(0);
	      	//Se crea un array con el número de resultados obtenidos
	      	 ...
	      	 //Recogemos el nombre de cada resultado
	      		for(int i=1; i<=num; i++){
	   			...
	   			//Se comprueba que el nombre recogido no se haya recogido antes
	        		...
	        		//Si no hay nombre duplicado lo guarda en el array
	      			if (sem==false){
			...
			protected void onProgressUpdate(Void... values) {
				...
				if (mostr==1){
					   try {
                           ArrayList<Titular2> mList = new ArrayList<Titular2>();
                           //Se añaden a la lista todos los archivos y carpetas de esa ruta
                           Titular2 titular;

                           for (int i = 0; i < mostrar.size(); i++) {
                               try {
                                   titular = new Titular2(mostrar.get(i));
                                   mList.add(titular);
                               } catch (Exception e) {

                               }
                           }
                               AdaptadorTitulares actualitatAdapter = new AdaptadorTitulares(Buscag.this, mList);
    ...
    public class AdaptadorTitulares extends BaseAdapter {
        ...
        private ArrayList<Titular2> mLItems;
        //Se asigna su activity y arraylist
        public AdaptadorTitulares(Activity a, ArrayList<Titular2> it) {
            try{
                mActivityAct = a;
                mLItems = it;
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (vi == null) {
                ...
                vh = new VistaH();
                vh.nombree = (TextView) vi.findViewById(R.id.Nombre);

                vi.setTag(vh);
            }
            //Se van añadiendo los contenidos de los arrays de cancion, grupo etc, a sus respectivos textviews
            vh = (VistaH) vi.getTag();
            Titular2 notice = mLItems.get(position);


            vh.nombree.setText(notice.getDescription());
